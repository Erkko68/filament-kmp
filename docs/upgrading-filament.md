...
| **Web / WASM** | **Karakum**-generated externals from `filament.d.ts` + overlay | `web/patches/` overlay **+** Kotlin `actual` |
...
- **`web/filament-js/jsbindings.cpp` → the JS surface.** The shipped `filament.d.ts`
  *under-reports* what is actually reachable at runtime — `jsbindings.cpp` is the real list of
  ...
  fix is our curated overlay in `web/patches/`. `scripts/dev/check-js-bindings.sh` audits this.
...
- **`filament.d.ts` (Web JS typed surface)** — its diff tells you what upstream *added to the
  typed surface*. Anything it adds that you already declare in `web/patches/filament.patch.d.ts`
...
- **Registered in `jsbindings.cpp`** (reachable at runtime): add the declaration to the overlay
  `web/patches/filament.patch.d.ts` so Karakum generates a typed external, then call it from the
  `webMain` actual. Instance methods are added by **declaration merging** — reopen the class as an
  ...
  `web/patches/filament.dts-overrides.json`. See [`web/README.md`](../web/README.md).)
- **Not in `jsbindings.cpp`** (no Web binding exists — e.g. `Engine.Builder` has no JS
  equivalent): make the `webMain` actual a no-op stub and mark it:
  ```kotlin
...
`web/patches/filament.patch.d.ts`, delete your now-redundant copy from the overlay. If upstream
...
| `:kotlin:*:jsTest` / `:kotlin:*:wasmJsTest` | Karakum regenerates externals from the new `filament.d.ts` + overlay; `webMain` compiles for both web targets |
...
ungenerated external would fail `webMain` compilation with an unresolved reference).
...
| `MaterialBuilder.coloredPenumbra(Boolean)` | C shim + 4 actuals (filamat has no JS binding → `webMain` no-op stub) |
| `Engine.Builder.colorGrading(ColorGrading.Builder)` | C shim + exposed `ColorGrading.Builder` handle as `internal` + 4 actuals (no JS `Engine.Builder` → `webMain` no-op stub) |
...
- The Web `Camutils$Manipulator` classes — added to upstream `filament.d.ts` (so Karakum picks
...
| JS overlay (instance methods, enums) | `web/patches/filament.patch.d.ts` |
| JS overrides (statics, field/typo fixes) | `web/patches/filament.dts-overrides.json` |
...
[`web/README.md`](../web/README.md) (Karakum pipeline),
