...
| **Web / WASM** | **Karakum**-generated externals from `filament.d.ts` + overlay | `js/patches/` overlay **+** Kotlin `actual` |
...
- **Registered in `jsbindings.cpp`** (reachable at runtime): add the declaration to the overlay
  `js/patches/filament.patch.d.ts` so Karakum generates a typed external, then call it from the
...
| `:kotlin:*:jsTest` | Karakum regenerates externals from the new `filament.d.ts` + overlay; `jsMain` compiles |
...
[`js/README.md`](../js/README.md) (Karakum pipeline),
