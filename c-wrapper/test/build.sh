...
echo "Building wrapper + progressive compile tests (engine + scene + renderer + view + camera + entity manager + fence + swapchain)..."
...
if [ "${FILA_ENABLE_LINKED_TESTS:-OFF}" = "ON" ]; then
  ...
  make test_program_engine_scene test_program_engine_scene_renderer test_program_engine_scene_view test_program_engine_scene_view_first_frame test_program_entity_manager test_program_engine_fence test_program_engine_swapchain
...
echo ""
echo "✅ Build successful! Progressive tests for Engine, Scene, Renderer, View, Camera, EntityManager, Fence, and SwapChain are in place."
