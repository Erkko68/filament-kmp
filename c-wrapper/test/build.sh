...
echo "Building wrapper + progressive compile tests (engine + scene + renderer + view + camera + entity manager + fence + swapchain + transform manager + light manager + renderable manager + vertex buffer + index buffer + material + texture + skybox)..."
...
echo ""
echo "✅ Build successful! Progressive tests for Engine, Scene, Renderer, View, Camera, EntityManager, Fence, SwapChain, TransformManager, LightManager, RenderableManager, VertexBuffer, IndexBuffer, Material, Texture, and Skybox are in place."
