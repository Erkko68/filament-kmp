...
### Added
- **Kotlin/Wasm web target (`wasmJs`)** (all modules): every artifact is now published for **both web flavors** — Kotlin/JS (`js`) and Kotlin/Wasm (`wasmJs`) — implemented once in a shared `webMain` source set over the same Karakum-generated Filament.js externals, with identical API surface and limitations on either target. The `webApp` sample runs on both. Includes wasm-runtime marshaling fixes and the Kotlin 2.4 `JsArray` type-collision fix in the web actuals.
