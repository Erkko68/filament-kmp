...
- **View option structs are data classes** (`filament`): 13 of the 14 nested `View.*Options` classes move to the `io.github.erkko68.filament` package as top-level `data class`es — `BloomOptions(enabled = true, strength = 0.4f)` now works, along with `copy()`, structural equality and `toString()`. Referenced as `BloomOptions` rather than `View.BloomOptions`. `View.RenderQuality` stays nested — one field, and the name would collide with filament-compose's own `RenderQuality`.
...
- **One out-parameter convention** (`filament`): every optional output buffer is now `out: T? = null`. `LightManager.getDirection`/`getPosition`/`getColor` no longer require an array, `TransformManager` and `RenderableManager` no longer require an explicit `null`, and `Scene.getEntities()`'s two overloads collapse into one.
...
### Fixed
- **`Renderer.FrameRateOptions` defaults disagreed across platforms** (`filament`): web answered `headRoomRatio = 1.0`, `scaleRate = 1.0` and `history = 1` where every other platform — and upstream — used `0.0`, `1/15` and `15`. One shared data class now holds one set of defaults.
- **`FogOptions`, `VignetteOptions` and `Ssct` compared their colour by reference** (`filament`): their `FloatArray` fields fell into the generated data-class `equals`, so two structs with identical colours were unequal — the exact recomposition bug the data classes were meant to fix. They compare contents now.
- **Android's `Renderer` option structs exposed the upstream object** (`filament`): `DisplayInfo.nativeInfo` and `FrameRateOptions`/`ClearOptions.nativeOptions` were public. They were body properties rather than constructor parameters, so the handle-sealing pass missed them, and the JVM-only api dumps could not catch them.
