...
- **`Light` escape hatch uses `vararg keys`** (`filament-compose`, source-breaking for `rebuildKey =` callers), matching `FilamentEffect` and `rememberMaterialInstance`; `rememberRenderTarget` renamed to `rememberRenderTargetTexture` (it returns the colour `Texture`; deprecated alias kept); `rememberEmissiveMaterialInstance` gained a default `intensity`; `CameraState` now throws when attached to two views at once instead of silently racing.
