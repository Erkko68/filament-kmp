...
### Added
- **Web bindings for what Filament 1.76.0 actually exposes** (`filament`, `gltfio`): `Fence`, `SkinningBuffer`, `MorphTargetBuffer` and the `RenderableManager` skinning/morphing/non-indexed-geometry builders now call the engine instead of throwing or no-op'ing; `MaterialInstance.material`/`duplicate`/`getConstant*`/`setScissor`, `Camera.getFieldOfViewInDegrees`, `LightManager.getComponentCount`, `Texture.isTextureSwizzleSupported`, `Renderer.copyFrame`/`readPixels`/`renderStandaloneView`/presentation-time setters, `Scene.skybox`/`indirectLight`, `View.viewport`/`shadowType`/`isFrustumCullingEnabled`/`isShadowingEnabled`/`hasCamera` and the VSM/soft-shadow/dynamic-resolution option structs are engine-backed, and `gltfio` gains `FilamentAsset.getMorphTargetNames`, `FilamentInstance.getSkinCount`/`getJointCountAt`/`getJointsAt` and a real `ResourceLoader` (the bound `gltfio$ResourceLoader` with the stb/ktx2/webp decoders) — see [Platform notes](docs/platform-notes.md) for what is still unbound.
...
- **Web API parity with the expanded JS bindings** (`filament`, `gltfio`, web only): on top of what 1.76.0 exposes, the web actuals now call the engine wherever upstream's expanded `jsbindings.cpp` has an entry point, instead of mirroring state in Kotlin or throwing — 207 more external declarations. `Material` reflection, the `ToneMapper` hierarchy, `ColorGrading.Builder.customLut`, `LightManager.Builder.shadowOptions`, `Texture.getTarget`/`getFormat`/`swizzle`/`external`, `View`'s option-struct getters and `visibleLayers`, `Camera.entity`, `IndexBuffer`/`VertexBuffer` counts, `RenderableManager.getMorphTargetCount`/`setMorphWeights`/`Builder.geometryType`, `SurfaceOrientation.Builder.tangents`, `SwapChain.setFrameScheduledCallback` and the `gltfio` instance/material accessors all reach the engine. `MaterialInstance` integer parameters marshal through `setIntParameter` rather than `setFloatParameter`.
- **`gltfio.ResourceLoader` and `UbershaderProvider` are real on web** (`gltfio`): `ResourceLoader` registers texture providers through the unified `addTextureProvider`, and `UbershaderProvider` reports its own material cache and forwards `needsDummyData` — only `createMaterialInstance`/`getMaterial` still throw, because both take a bitfield-packed `MaterialKey` that embind cannot marshal.
...
- **[Platform notes](docs/platform-notes.md) cover the remaining web gaps**: `Engine.isValid`, non-blocking `Fence.wait`, browser-managed frame pacing and both `SwapChain` frame callbacks.
...
- **`Engine.config` reported defaults on JVM and iOS** (`filament`): the C wrapper has no `getConfig`, and both actuals answered a fresh `Config()` rather than the one the `Builder` was given. `Builder.build()` hands the Config to the Engine now.
- **`Filamat.init()` did nothing on iOS** (`filamat`): the native actual was an empty body while `shutdown()` still called through, so every `build()` returned `Package::invalidPackage()` and each unpaired `shutdown()` tore down glslang's process state again — enough repetitions segfaulted. `MaterialBuilderTest` now asserts the package is valid rather than merely non-null.
- **Sampler and subpass material parameters reported the wrong type on JVM and iOS** (`filament`): `Material.parameters` read `ParameterInfo.type` as a `UniformType` without checking `isSampler`/`isSubpass`, which select the other two arms of a union — so a `sampler2d` came back as `BOOL`. JVM and iOS share one decoder now; Android's flattening already came from the AAR.
- **`Renderer.FrameRateOptions` defaults disagreed across platforms** (`filament`): web answered `headRoomRatio = 1.0`, `scaleRate = 1.0` and `history = 1` where every other platform — and upstream — used `0.0`, `1/15` and `15`.
- **Android's option structs exposed their upstream object** (`filament`): the `native`/`nativeInfo`/`nativeOptions` field on each of `View`'s 14 option structs and `Renderer`'s 3 was public. They are body properties rather than constructor parameters, so the handle-sealing pass missed them, and the JVM-only api dumps could not catch them.
...
- **Web `Engine.destroy` leaked its canvas and GL context** (`filament`): the hidden canvas `Engine.create` parks on the document stayed live, so repeated create/destroy walked into the browser's cap on WebGL contexts and later engines aborted.
- **Texture formats fell back to `RGBA8` on web** (`filament`): `Texture.Builder.format` mapped 10 of 101 `InternalFormat` values, so a `DEPTH24_STENCIL8` texture was built as colour. The JS enum registers Filament's `TextureFormat` in declaration order, so the two now map by ordinal.
- **`View.FogOptions.densityMap` was never a Filament field** (`filament`, **source-breaking**): invented in our binding and silently dropped on every platform. Removed — `Options.h` has no such field.
