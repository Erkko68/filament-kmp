...
- **Shared `CameraController` interface** (`filament-compose`): `OrbitCameraState`/`MapCameraState`/`FlightCameraState` now implement a common `CameraController` (`setViewport`/`resetToHome`/`saveBookmark`/`jumpToBookmark`), so generic UI can reset or bookmark whichever camera is active. `FlightCameraState` gained `resetToHome`/bookmarks (Filament's `Manipulator` supports FLIGHT bookmarks); its per-frame `update` stays flight-only.
...
- **Loader `engine` parameter moved last** (`filament-compose`, source-breaking for positional callers): `rememberMaterial`/`rememberTexture`/`rememberGltfAsset` now take their defaulted `engine` just before the trailing `load` lambda instead of first, so a defaulted `engine` is consistently the last value parameter across the DSL (the builders like `rememberColorMaterialInstance` already were). Pass it by name — `rememberGltfAsset(engine = engine) { … }` — since `engine` positional-first now silently binds as the reload `key`. `rememberKTXEnvironment`/`rememberHDREnvironment` keep `engine` first and required: they run outside `rememberFilamentScene { }` where `LocalFilamentEngine` isn't in scope.
- **All gesture modifiers are plain `Modifier` extensions** (`filament-compose`): `Modifier.flightGestures` is no longer `@Composable` — its `FocusRequester` moved into `FlightCameraState`, so `orbitGestures`/`mapGestures`/`flightGestures` share one non-composable shape, matching Compose's own `draggable`/`scrollable`.
