...
- **Flight camera no longer jumps on tuning changes** (`filament-compose`): `rememberFlightCameraState` now carries the current pose across a tuning-parameter rebuild like orbit/map — previously changing e.g. `maxMoveSpeed` mid-flight snapped orientation back to `startPitch`/`startYaw`.
