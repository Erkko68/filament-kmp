...
### Added
- **Web API parity with the expanded JS bindings** (`filament`, `gltfio`, web only): the web actuals now call the engine wherever upstream's expanded `jsbindings.cpp` exposes an entry point, instead of mirroring state in Kotlin or returning a placeholder. `View` drops 25 cached fields to 4 (only `scene`/`camera`/`renderTarget`/`colorGrading` stay cached, for wrapper identity, exactly as the JVM and Android actuals do) — every option struct, `name`, `viewport`, `visibleLayers`, `dithering`, `antiAliasing`, `shadowType`, `isPostProcessingEnabled`, `isScreenSpaceRefractionEnabled` and `getLastDynamicResolutionScale` are engine-backed, and the option setters push the fields they had been dropping (bloom `quality`/`lensFlare`/`ghost*`/`halo*`, DoF ring counts, AO `quality`/`bentNormals`). `Material`'s reflective API, `MaterialInstance.getConstant*`/`isStencilWriteEnabled`, `ColorGrading.Builder.customLut`, the full `ToneMapper` hierarchy, `Camera.entity`, `LightManager.destroy`, `TransformManager.getChildCount`/`isAccurateTranslationsEnabled`, `Texture.getTarget`/`getFormat`/`Builder.samples`/`swizzle`/`external`, `Skybox.Builder.intensity`, `Scene.skybox`/`indirectLight`, and the `IndexBuffer`/`VertexBuffer` counts all reach the engine now. `gltfio.ResourceLoader` is a real `gltfio$ResourceLoader` with the Stb and Ktx2 texture providers registered, so `asyncGetLoadProgress()` reports real progress.

### Fixed
- **Integer material parameters were marshalled as floats on web** (`filament`): `MaterialInstance.setParameter(name, Int…)` — scalar and the `int2`/`int3`/`int4` and `IntElement` array forms — all routed through `setFloatParameter`, because the integer entry points were not bound; an `int` uniform silently received a float. They now use `setIntParameter`/`setInt{2,3,4}Parameter`, and boolean vectors use `setBool{2,3,4}Parameter`.
- **Web `Material` reflection returned invented values** (`filament`): `getShading()` always answered `LIT`, `getBlendingMode()` `OPAQUE`, `getParameterCount()` `0` and `getParameters()` an empty list — so `hasParameter()`, which searched that list, always returned `false`. Every one of these now queries the material.
- **Web tone mappers silently degraded** (`filament`): `PBRNeutralToneMapper`, `GT7ToneMapper` and `Agx` all fell back to ACES, and `Generic` ignored its contrast/mid-gray/hdrMax parameters. Each now constructs its own operator, and `Generic`'s properties read and write through.
- **Web texture formats fell back to `RGBA8`** (`filament`): `Texture.Builder.format` mapped only 10 of the 109 `InternalFormat` values and silently substituted `RGBA8` for the rest (so e.g. a `DEPTH24_STENCIL8` texture was built as colour). The mapping is now exact in both directions — the JS enum registers Filament's `TextureFormat` in declaration order, so the 101 shared entries line up by ordinal.
- **`AssetLoader.destroy` leaked the loader on web** (`gltfio`): it called `delete()`, whose embind `raw_destructor` for `AssetLoader` is a no-op; it now calls the static `destroy` that actually frees it.
