...
@Composable
internal fun FilamentViewWindows(modifier: Modifier, controller: FilamentController) {
        ...
        if (pixelBuffer != null) {
            Texture.PixelBufferDescriptor(
                ...
                callback = {
                    ...
                    if (oldImage == null) {
                        println("FilamentViewWindows: First frame read successfully (${textureSize.width}x${textureSize.height})")
                    }
                }
            )
        } else null
    ...
    FilamentRenderLoop { frameTime ->
        ...
        if (!readPixelsPending && pixelBufferDescriptor != null && textureSize.width > 0 && textureSize.height > 0) {
            ...
            // CRITICAL: Ensure the GPU target size matches our CPU buffer size to avoid BufferOverflowException during resize
