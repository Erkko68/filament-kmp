...
public class HDRLoader {
     ...
     */
    @Nullable
    public static Texture createTexture(@NotNull Engine engine, @NotNull java.nio.Buffer buffer, @NotNull Texture.InternalFormat internalFormat) {
        long nativeEngine = engine.getNativeObject();
        long nativeTexture = nCreateHDRTexture(nativeEngine, buffer, buffer.remaining(), internalFormat.ordinal());
        if (nativeTexture == 0L) {
            return null;
        }
    ...
    private static native long nCreateHDRTexture(long nativeEngine, java.nio.Buffer buffer, int remaining, int format);
}
