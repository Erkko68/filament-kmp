...
AutoBuffer::AutoBuffer(JNIEnv *env, jobject buffer, jint size, bool commit) noexcept :
    ...
    jmethodID getBaseArrayOffset = env->GetStaticMethodID(nioUtilsClass,
            "getBaseArrayOffset", "(Ljava/nio/Buffer;I)I");
    jmethodID getBufferType = env->GetStaticMethodID(nioUtilsClass,
            "getBufferType", "(Ljava/nio/Buffer;)I");

    mBuffer = env->NewGlobalRef(buffer);

    mType = (BufferType) env->CallStaticIntMethod(nioUtilsClass, getBufferType, mBuffer);
    ...
    if (address) {
        // Direct buffer case
        mData = reinterpret_cast<void *>(env->CallStaticLongMethod(nioUtilsClass,
                getBasePointer, mBuffer, address, mShift));
        mUserData = mData;
    } else {
        // wrapped array case
        jarray array = (jarray) env->CallStaticObjectMethod(nioUtilsClass, getBaseArray, mBuffer);
        jint offset = env->CallStaticIntMethod(nioUtilsClass, getBaseArrayOffset, mBuffer, mShift);
