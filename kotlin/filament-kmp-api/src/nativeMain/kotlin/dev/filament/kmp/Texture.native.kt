...
actual class Texture internal constructor(internal var nativeHandle: CPointer<FilaTexture>?) {
    ...
    actual fun setImage(engine: Engine, level: Int, xoffset: Int, yoffset: Int, zoffset: Int, width: Int, height: Int, depth: Int, descriptor: PixelBufferDescriptor) {
        ...
        when (val storage = descriptor.storage) {
            is FloatArray -> { val p = storage.pin(); pinned = p; ptr = p.addressOf(0) }
            is ByteArray -> { val p = storage.pin(); pinned = p; ptr = p.addressOf(0) }
            is ShortArray -> { val p = storage.pin(); pinned = p; ptr = p.addressOf(0) }
            is IntArray -> { val p = storage.pin(); pinned = p; ptr = p.addressOf(0) }
        }

        if (pinned != null || descriptor.callback != null) {
            val wrapper = PixelBufferPinWrapper(pinned ?: ByteArray(0).pin(), descriptor.callback)
