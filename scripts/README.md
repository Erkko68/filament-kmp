...
| [`download_filament_includes.py`](download_filament_includes.py) | Downloads Filament's public C/C++ headers from the GitHub source tarball into `include/`. **Required by both the native (cinterop) and JNI builds** — see [c/CMakeLists.txt](../c/CMakeLists.txt) and [java/CMakeLists.txt](../java/CMakeLists.txt). Headers must match the prebuilt ABI exactly — version drift causes runtime crashes. Also synthesizes a cross-platform `gltfio/materials/uberarchive.h` by pulling `UBERARCHIVE_DEFAULT_SIZE` from each per-platform release tarball (the source tarball doesn't ship this auto-generated header). Invoked by the `downloadIncludes` Gradle task. |
