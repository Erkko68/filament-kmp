...
# externals declared in this repo's `*.web.kt` files.
#
# Why this exists:
#   The Kotlin externals under web/src/webMain are hand-maintained against
#   `jsbindings.cpp` (the *actual* binding source — upstream's filament.d.ts
#   drifts behind it). A binding present in jsbindings.cpp but absent from the
#   externals can't be called from Kotlin. This script finds those gaps so the
#   external declarations can be added.
#
# What this checks:
...
#   (kotlin/*/src/webMain/.../<JsName>.web.kt — with `$Builder` mapped to a
#   nested `Builder` class inside the parent's file).
#
#   Prints two lists:
#     1. MISSING — bindings present upstream but not referenced anywhere in
#        our `*.web.kt` files. These are stubs you can implement.
#     2. ORPHANED — references in `*.web.kt` to JS function names that don't
...
KOTLIN_GLOB="$REPO_ROOT/kotlin/*/src/webMain $REPO_ROOT/kotlin/*/src/jsMain $REPO_ROOT/kotlin/*/src/wasmJsMain"
# The committed external declarations in :web are the "declared surface" we
# audit jsbindings.cpp against. A binding present upstream but absent from them
# is a declaration to add under web/src/webMain.
DECLARED_SOURCES_GLOB="$REPO_ROOT/web/src/webMain"
...
# so we can distinguish "missing from the externals entirely" (actionable: add a
...
EXT_TOKENS="$(grep -rohE '[A-Za-z_][A-Za-z0-9_]*' $DECLARED_SOURCES_GLOB --include='*.kt' 2>/dev/null | sort -u)"
...
echo "===== MISSING IN EXTERNALS — declare these in web/src/webMain ====="
...
echo
echo "===== ORPHANED — calls in *.web.kt that don't match any jsbindings name  ====="
...
if [[ $MISSING_EXT_COUNT -gt 0 ]]; then
  echo "Tip: add the missing declarations under web/src/webMain with the correct"
  echo "signature from web/filament-js/jsbindings.cpp."
fi
